...
```bash
ToDo-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ToDo-app$ sam local start-api
ToDo-app$ curl http://localhost:3000/
```
...
```bash
ToDo-app$ sam logs -n HelloWorldFunction --stack-name ToDo-app --tail
```
...
```bash
ToDo-app$ pip install pytest pytest-mock --user
ToDo-app$ python -m pytest tests/ -v
```
